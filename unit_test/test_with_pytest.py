...
def test_map_categorical_vars():
    ...
    cnx = sqlite3.connect(DB_PATH+DB_FILE_NAME)
    categorical_variables_mapped_df = pd.read_sql('select * from categorical_variables_mapped', cnx)
    ...
    assert test_case.equals(categorical_variables_mapped_df)
...
def test_interactions_mapping():
    ...
    cnx = sqlite3.connect(DB_PATH+DB_FILE_NAME)    
    interactions_mapped_df = pd.read_sql('select * from interactions_mapped', cnx)
    ...
    assert test_case.equals(interactions_mapped_df)